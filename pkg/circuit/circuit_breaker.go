// Package circuit implements the circuit breaker pattern for LLM providers.
//
...
func NewCircuitBreaker(providerID string, p provider.LLMProvider, config CircuitBreakerConfig) *CircuitBreaker {
...
func NewDefaultCircuitBreaker(providerID string, p provider.LLMProvider) *CircuitBreaker {
	return NewCircuitBreaker(providerID, p, DefaultCircuitBreakerConfig())
}
...
func (cbm *CircuitBreakerManager) Register(providerID string, p provider.LLMProvider) *CircuitBreaker {
	cbm.mu.Lock()
	defer cbm.mu.Unlock()

	cb := NewCircuitBreaker(providerID, p, cbm.config)
