...
// bug in the Qwen SSE read loop: `line, err := reader.ReadBytes('\n'); if err !=
// nil { if err == io.EOF { break } }` DROPS the final partial line that ReadBytes
// returns alongside io.EOF when the last `data: {...}` chunk has no trailing
// newline.
//
// baseURL contains "compatible-mode" so getAPIEndpoint() resolves to the
// OpenAI-shaped /chat/completions endpoint matching this chunk shape.
func TestCompleteStream_EOFLastChunkNoNewline(t *testing.T) {
	...
	provider := NewQwenProvider("test-api-key", server.URL+"/compatible-mode/v1", "qwen-turbo")
