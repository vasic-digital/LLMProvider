...
// the loop must process it before breaking. xai emits per-delta chunks AND a final
// aggregate on [DONE]; here the stream ends in EOF (no [DONE]), so the dropped
// final delta is lost content for the consumer.
